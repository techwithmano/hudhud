...
export default function FinalScene({ onRestart }: FinalSceneProps) {
  return (
    <motion.div
      ...
      className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-gradient-to-br from-background to-secondary p-4 sm:p-8"
    >
      <motion.div
        initial={{ y: -20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 0.5, duration: 1.5 }}
        className="max-w-2xl w-full bg-card/80 backdrop-blur-sm p-6 sm:p-8 rounded-xl shadow-lg"
      >
        <div className="max-h-[80vh] overflow-y-auto text-left text-foreground font-body">
            <h2 className="text-2xl md:text-3xl font-bold mb-4 text-primary text-center">To My Nooni 🌷</h2>
            ...
            <p className="mt-6 text-right text-base md:text-lg">— From me, always 💙</p>
        </div>
      </motion.div>
    </motion.div>
  );
